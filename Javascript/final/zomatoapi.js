...
function getResID(lat,lon) {
    ...
    function updatePage(locationResults){ 
        // GETS restaraunts  ID
        for(let i = 0; i < locationResults.nearby_restaurants.length; i++) {    
  ...
  var queryURL = "https://developers.zomato.com/api/v2.1/restaurant?res_id=" + res_id;
    ...
    function updatePage(resDetails){
      ...
      if(ifDeliver === "Delivery"){
        ifDeliver = "Delivery is available.";
      }else{
...
function appendINFO() {
  ...
  // var addressINFO = $("<p>");
  // addressINFO.attr("id", "address");
  // addressINFO.text("Address: "+ address);
  // resINFO.append(addressINFO);
}
